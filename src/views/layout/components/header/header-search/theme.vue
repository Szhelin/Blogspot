  ...
  <div class="">
    ...
    <div class="flex h-[140px]" v-if="themeData.list.length">
      ...
      >
        ...
        />
        <p
          class="absolute bottom-0 left-0 w-full h-full flex items-center backdrop-blur rounded px-1 text-white text-sm duration-300 hover:backdrop-blur-none"
        >
          # {{ themeData.big.title }}
        </p>
      </div>
      <div class="flex flex-wrap flex-1 max-w-[860px]">
        <div
          ...
          :key="item.id"
          class="h-[45%] w-[260px] text-white text-sm relative ml-1.5 mb-1.5 rounded"
          :style="{
            backgroundColor: randomRGB()
          }"
        >
